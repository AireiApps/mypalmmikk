...
@Component({
  selector: 'app-managerorderreq-add',
  templateUrl: './managerorderreq-add.page.html',
  styleUrls: ['./managerorderreq-add.page.scss'],
})
export class ManagerorderreqAddPage implements OnInit {
  ...
  constructor(private router: Router, private fb: FormBuilder, private service: ManagerServiceService, private commonservice: AIREIService) { 
  ...
  btn_save() {
    ...
    this.service.saveSOR(req).then((result) => {
      ...
      if (resultdata.httpcode == 200) {
        ...
        this.router.navigate(["/managerorderreqlist"]);
      } else {
