...
@Component({
  selector: 'app-securitycommunication-put',
  templateUrl: './securitycommunication-put.page.html',
  styleUrls: ['./securitycommunication-put.page.scss'],
})
export class SecuritycommunicationPutPage implements OnInit {
    ...
    this.imgUpload.ImageUploadSupervisor(areq).then(
      (result) => {
        ...
        if (resultdata.httpcode == 200) {
          ...
          {
            this.imagePaths.broadcastcommunication_path = resultdata.data.uploaded_path;
          }else{
            this.imagePaths.personalizedcommunication_path = resultdata.data.uploaded_path;
          }
    ...
    if(type == "BroadcastCommunication")
    {
      if (this.broadcastcommunicationForm.valid) {
        ...
        this.service.saveBroadCastCommunication(req).then((result) => {
          ...
          if (resultdata.httpcode == 200) {
            ...
            this.commonservice.presentToast("Broadcast Communication Saved Successfully!");

            this.router.navigate(["/broadcastcommunicationreport"]);
          } else {
      ...
      if (this.personalizedcommunicationForm.valid) 
      {
        ...
        var areq = {
          ...
          comments : this.personalizedcommunicationForm.value.personalizedComments
        };
        ...
        this.service.savePersonalizedCommunication(areq).then((result) => {
          ...
          if (resultdata.httpcode == 200) {
            ...
            this.commonservice.presentToast("Personalized Communication Saved Successfully!");

            this.router.navigate(["/personalizedcommunicationreport"]);
          } else {
            this.commonservice.presentToast("Saved Failed!");
          }        
        });
