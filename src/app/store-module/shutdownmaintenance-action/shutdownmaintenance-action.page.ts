...
@Component({
  selector: 'app-shutdownmaintenance-action',
  templateUrl: './shutdownmaintenance-action.page.html',
  styleUrls: ['./shutdownmaintenance-action.page.scss'],
})
export class ShutdownmaintenanceActionPage implements OnInit {
  ...
  constructor(private router: Router, private fb: FormBuilder, private service: StoreServiceService, private commonservice: AIREIService, private route: ActivatedRoute) {
    ...
    this.sdmpdetailForm = this.fb.group({
      ...
      sdmpaction: new FormControl('', Validators.required),
  ...
  btn_save()
  {
    if(this.sdmpdetailForm.valid)
    {
      let req = {
        shutdown_id: this.params.shutdown_id,
        millcode: this.userlist.millcode,
        action: this.sdmpdetailForm.value.sdmpaction,
        comments : this.sdmpdetailForm.value.taComments
      }
      //console.log(req);
  
      this.service.saveShutDownMaintenanceAction(req).then(result => {
        var resultdata: any;
        resultdata = result;      
        if (resultdata.httpcode == 200) {
