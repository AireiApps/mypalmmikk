...
@Component({
  selector: "app-store-order-request",
  templateUrl: "./store-order-request.component.html",
  styleUrls: ["./store-order-request.component.scss"],
})
export class StoreOrderRequestComponent implements OnInit {
  storeform;
  requestList = [];
  selectedUnit='';
  stationArr = [];
  ...
  btn_save() {
    ...
    let normal = Number(this.storeform.value.normal);
    let urgent = Number(this.storeform.value.urgent);
    let type=0;
