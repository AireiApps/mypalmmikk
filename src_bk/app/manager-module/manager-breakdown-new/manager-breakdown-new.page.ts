...
@Component({
  selector: "app-manager-breakdown-new",
  templateUrl: "./manager-breakdown-new.page.html",
  styleUrls: ["./manager-breakdown-new.page.scss"],
})
export class ManagerBreakdownNewPage implements OnInit {
    ...
    if (this.newbreakdowndowntimeForm.valid) {
      ...
      this.service.savebreakdowndowntime(req).then((result) => {
        ...
        if (resultdata.httpcode == 200) {
          ...
          this.router.navigate(["/manager-breakdown-list"]);
