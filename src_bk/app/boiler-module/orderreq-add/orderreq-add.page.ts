...
@Component({
  selector: 'app-orderreq-add',
  templateUrl: './orderreq-add.page.html',
  styleUrls: ['./orderreq-add.page.scss'],
})
export class OrderreqAddPage implements OnInit {
  ...
  constructor(private router: Router, private fb: FormBuilder, private service: BoilerServiceService, private commonservice: AIREIService) {
  ...
  btn_save() {
    ...
    this.service.saveSOR(req).then((result) => {
      ...
      if (resultdata.httpcode == 200) {
        ...
        this.router.navigate(["/orderreqlist"]);
      } else {
  ...
  onChangeItem(event) {
    ...
    //this.storeform.controls.unit.setValue("");

    const eachitem = JSON.parse(event.target.value);

    //this.commonservice.presentToast("Selected Unit: " + eachitem.item_unit);
